...
# Import the advanced detectors from extra_WIP
try:
    import sys
    import os
    
    # Add extra_WIP path to sys.path
    extra_wip_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "extra_WIP")
    if extra_wip_path not in sys.path:
        sys.path.append(extra_wip_path)
    
    from risk_detection.detectors.hallucination_detector import (
        HallucinationDetector, 
        HallucinationType, 
        HallucinationResult,
        create_hallucination_detector
    )
    from risk_detection.detectors.adversarial_detector import (
        AdversarialDetector, 
        AdversarialType, 
        AdversarialResult,
    ...
    logger.warning(f"⚠️ Advanced detectors not available: {e}")
    ...
    class HallucinationResult:
        ...
        flagged_segments: List[Dict[str, Any]] = None
        
    @dataclass
    class AdversarialResult:
        is_adversarial: bool = False
        confidence_score: float = 0.0
        attack_type: Optional[AdversarialType] = None
        risk_level: str = "LOW"
        evidence: Dict[str, Any] = None
        flagged_patterns: List[Dict[str, Any]] = None
    ...
    def __init__(self):
        ...
        if ADVANCED_DETECTORS_AVAILABLE:
            try:
                ...
                self.adversarial_detector = create_adversarial_detector({
                ...
                logger.error(f"❌ Failed to initialize enhanced detectors: {e}")
                self.hallucination_detector = None
                self.adversarial_detector = None
