  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const endpoints = Object.keys(analyticsApi);
        console.log("Available analytics endpoints:", endpoints);
        
        // Try to use the most appropriate endpoints based on what's available
        if (endpoints.includes('statistics') && endpoints.includes('timeline') && endpoints.includes('realTimeStats')) {
          console.log("Using standard analytics endpoints");
          ...
          console.log('Real-time Response:', realTimeResponse.data);
          ...
          console.log("Using dashboard endpoint fallback");
          const dashboardResponse = await analyticsApi.getDashboardOverview(30);
          console.log('Dashboard Response:', dashboardResponse.data);
          
          // Map the response to our expected format
          if (dashboardResponse.data) {
            setStats({
              total_requests: dashboardResponse.data.today_stats?.total_requests || 0,
              high_risk_requests: dashboardResponse.data.today_stats?.high_risk_count || 0,
              avg_risk_score: dashboardResponse.data.today_stats?.avg_risk_score || 0,
              blocked_requests: dashboardResponse.data.today_stats?.blocked_requests || 0,
              total_users: dashboardResponse.data.today_stats?.total_users || 0,
              active_users: dashboardResponse.data.today_stats?.active_users || 0
            });
            
            setTimeline(dashboardResponse.data.timeline || []);
            
            // Create a mock real-time stats object if needed
            setRealTimeStats({
              current_requests_per_minute: dashboardResponse.data.today_stats?.requests_per_minute || 0,
              active_users: dashboardResponse.data.today_stats?.active_users || 0,
              avg_response_time: dashboardResponse.data.today_stats?.avg_response_time || 0,
              system_health: dashboardResponse.data.system_health || 'operational',
